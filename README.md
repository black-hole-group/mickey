...
- [OPTIONAL] PGI compiler (`pgcc`) compiler to make use of GPU or CPU acceleration for fast array handling. Only required for regridding (cf. [C extensions](#c-extensions)).
...
Note that you need the PGI compiler and a NVIDIA GPU. GPUs can speed up regridding by a factor of 100! The compiler also supports using a multicore CPU, though the makefile needs to be changed (it is written for a GPU compilation).
...
- [ ] write a CPU only version for branch openacc
- [ ] more examples
